...
struct MobileTarget
{
	...
	bool uses_vector_for_float() const
	{
		return true;
	}
};
...
public:
	...
	BenchResult bench_computation_chain(std::uint32_t chain_length, bool use_floats)
	{
		...
		arc::RegisterClass target_class = use_floats ? arc::RegisterClass::VECTOR : arc::RegisterClass::GENERAL_PURPOSE;
		...
		result.gp_pressure_ratio = static_cast<double>(allocator.pressure(region, arc::RegisterClass::GENERAL_PURPOSE))
		                           / 13.0;
		result.vec_pressure_ratio = static_cast<double>(allocator.pressure(region, arc::RegisterClass::VECTOR)) / 16.0;

		std::uint32_t total_values = result.gp_allocated + result.vec_allocated + result.gp_spilled + result.
		                             vec_spilled;
		result.spill_ratio = total_values > 0
			                     ? static_cast<double>(result.gp_spilled + result.vec_spilled) / total_values
			                     : 0.0;
		...
		for (auto *ir_node: all_computations)
		{
			...
			if (dag_node && dag_node->kind == arc::NodeKind::VALUE && dag_node->value_t != arc::DataType::VOID)
			{
				...
				arc::RegisterClass expected_class = is_float
					                                    ? arc::RegisterClass::VECTOR
					                                    : arc::RegisterClass::GENERAL_PURPOSE;
	...
	BenchResult bench_control_flow_merge()
	{
		...
		if (dag_from && dag_final)
		{
			...
			if (from_result.allocated())
			{
				...
				if (used_caller_saved)
					result.from_nodes_optimized++;
			}
			if (final_alloc.allocated())
				result.gp_allocated++;
			if (from_result.spilled)
				result.gp_spilled++;
			if (final_alloc.spilled)
				result.gp_spilled++;
		}

		result.gp_pressure_ratio = static_cast<double>(allocator.pressure(merge, arc::RegisterClass::GENERAL_PURPOSE)) /
		                           13.0;
